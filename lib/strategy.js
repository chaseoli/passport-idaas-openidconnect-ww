...
function Strategy(options, verify) {
  ...
  if (options.addCACert) {
    ...
    // decode buffer from base64 and write to certs array
    for (let i = 0; i < options.CACertPathListBase64.length; i++) {
      const buff = Buffer.from(options.CACertPathListBase64[i], 'base64')
      this._certs.push(buff);
    }
